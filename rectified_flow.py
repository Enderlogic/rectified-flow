...
class RectifiedFlow_VS:
    ...
    # v1.2: reflow增加x_0的输入
    def create_flow(self, x_1, t, x_0=None):
        """ 使用x_t = t^k * x_1 + (1 - t^k) * x_0公式构建x_0到x_1的流
        ...
        # 获得xt的值
        x_t = t ** self.k * x_1 + (1 - t ** self.k) * x_0

        return x_t, x_0

    # 司机
    def mse_loss(self, v, x_1, x_0, t):
        """ 计算RectifiedFlow的损失函数
        L = MSE(x_1 - x_0 - v(t))  匀速直线运动
        ...
        # 求loss函数，是一个MSE，最后维度是[B]
        loss = F.mse_loss(self.k * t[:, None, None, None] ** (self.k - 1) * (x_1 - x_0), v)
        # loss = torch.mean((x_1 - x_0 - v)**2)
